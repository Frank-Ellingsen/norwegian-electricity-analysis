...
def save_hydapi_data(df: pd.DataFrame, output_path: str):
    ...
    if os.path.exists(output_path):
        ...
        combined_df = pd.concat([existing_df, df]).drop_duplicates(subset=['station_id', 'timestamp', 'parameter_id'], keep='last')
        combined_df.to_csv(output_path, index=False)
    else:
        df.to_csv(output_path, index=False)
    
    logging.info(f"Saved {len(df)} HydAPI rows to {output_path}")
